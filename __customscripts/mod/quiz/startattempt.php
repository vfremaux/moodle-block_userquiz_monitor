...
// CHANGE+ : Check userquiz_monitor component after access control.
$uqmanager = null;
if (is_dir($CFG->dirroot.'/blocks/userquiz_monitor')) {
    ...
    if ($uqmanager->is_training_or_exam($quizobj->get_quizid())) {
        $PAGE->add_body_class('is-userquiz-monitor-quiz');

        if ($uqmanager->is_training($quizobj->get_quizid())) {
            $PAGE->add_body_class('is-userquiz-monitor-training');
        } else {
            $PAGE->add_body_class('is-userquiz-monitor-exam');
        }
    }
}
// CHANGE-.
...
// CHANGE+ : If userquiz monitor quiz, then register attempt in examtraining stats compiler.
if (!empty($uqmanager)) {
    if ($uqmanager->is_training_or_exam($quizobj->get_quizid())) {
        $uqmanager->register_attempt($attempt);
    }
}
// CHANGE-
